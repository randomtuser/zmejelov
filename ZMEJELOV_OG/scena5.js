class scena5 extends Phaser.Scene {
	constructor(){
		super({ key: 'scena5' });
	}
	 preload(){
...
this.physics.add.overlap(gameState.junak2, gameState.sovraznik2, () => {
		this.add.text(500, 1000, '      Game Over...\n  klikni na zaslon in poskusi znova', { fontFamily: 'Arial', fontSize: 36, color: '#ffffff' });
	...
	this.physics.add.overlap(gameState.junak2, gameState.sovraznik3, () => {
			this.add.text(500, 150, '      Game Over...\n  klikni na zaslon in poskusi znova', { fontFamily: 'Arial', fontSize: 36, color: '#000000' });
			gameState.junak2.destroy()
			this.physics.pause();
			gameState.active = false;
			this.anims.pauseAll();
...
this.physics.add.overlap(gameState.junak2, izhod5, () => {
					      this.add.text(150, 350, 'lol, to ni izhod', { fontFamily: 'Arial', fontSize: 36, color: '#000000' });

					    })
this.physics.add.overlap(gameState.junak2, REALizhod5, () => {
												      this.add.text(250, 1300, 'Vstat od mrtvih je težko, h?!\n  Klikni na zaslon za naslednji level', { fontFamily: 'Arial', fontSize: 36, color: '#45664464' });
												      this.physics.pause();
												      gameState.active = false;
															//za lvl 4
															this.input.on('pointerup', () => {
