class scena6 extends Phaser.Scene {
	constructor(){
		super({ key: 'scena6' });
	}
	 preload(){
...
this.physics.add.overlap(gameState.junak2, gameState.sovraznik2, () => {
	...
	this.physics.add.overlap(gameState.junak2, gameState.sovraznik3, () => {
		...
		this.physics.add.overlap(gameState.junak2, tilesLobanjaKill, () => {
				this.add.text(500, 800, '      Game Over...\n klikni na zaslon in poskusi znova', { fontFamily: 'Arial', fontSize: 36, color: '#ffffff' });
				this.physics.pause();
				gameState.active = false;
				this.anims.pauseAll();
				this.input.on('pointerup', () => {
...
this.physics.add.overlap(gameState.junak2, gameState.spaceship, () => {
												      this.add.text(500, 250, 'Jej, uspelo ti je!! Klikni na zaslon', { fontFamily: 'Arial', fontSize: 36, color: '#FFF0F5' });
												      this.physics.pause();
												      gameState.active = false;
															//za lvl 4
															this.input.on('pointerup', () => {
